...
def fit_and_forecast_main(fit_df, predict_df, metric, order):
    """
    ...
    exog_forecast = predict_df[['days_since_last_game', 'season_type']].loc[:, exog_df.columns]
    preds = arima_model_fit.forecast(steps=predict_df.shape[0], exog=exog_forecast, information_set="predicted").reset_index(drop=True)


    predict_df[f"predicted_{metric}"] = preds

    return predict_df

# Function to fit ARIMA model and make one-day out-of-sample forecast
def fit_and_forecast_player(league_game_df, prediction_date, metric, order):
    """
    Fit ARIMA model to data up to a specified date and make a one-day out-of-sample forecast.
...
def main(metric='league_avg_fg3a_fga', train_seasons=range(2010, 2015), val_seasons=range(2015, 2016), test_seasons=range(2016, 2020)):

    # Preprocess the data
    league_game_df = league_data_loader(seasons=list(train_seasons) + list(val_seasons) + list(test_seasons))

    # Fit and summarize the ARIMA model
    train_league_df = league_game_df[league_game_df['season'].isin(train_seasons)]
    val_test_df = league_game_df[league_game_df['season'].isin(list(val_seasons) + list(test_seasons))].reset_index(drop=True)
    fit_and_summarize_arima(train_league_df, metric=metric, order=(4, 1, 0))

    predictions_df = fit_and_forecast_main(train_league_df, val_test_df, metric=metric, order=(4, 1, 0))

    predictions_df = predictions_df[predictions_df.season.isin(test_seasons)]
...
def player_main(athlete_name,  order, train_seasons=range(2016, 2018), test_seasons=range(2018, 2020), metric='fg3a_fga', league_model=None):
    ...
    if league_model is None:
        ...
        df = pd.merge(df, league_game_df[["game_date", f'predicted_league_avg_{metric}']], on='game_date', how='inner')
    ...
    for i in range(len(unique_dates) - 1):
        ...
        predictions[prediction_date] = fit_and_forecast_player(df, prediction_date, metric=f"{athlete_name}_{metric}_delta", order=order)
...
if __name__ == "__main__":
   ...
   # player_main(athlete_name="Brook Lopez", order=(4, 0, 0), league_model=("CNN", "cnn_test_predictions.csv"))
   ...
   # player_main(athlete_name="Anthony Davis", order=(1, 0, 1), league_model=("CNN", "cnn_test_predictions.csv"))
